...
public class GalleryLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        LogUtil.d(TAG, "onLayoutChildren() called with: state = [" + state + "]");
        if (getItemCount() == 0) {
            reset();
        ...
        if (state.getItemCount() != 0 && !state.didStructureChange()) {
            LogUtil.d(TAG, "onLayoutChildren: ignore extra layout step");
            return;
        }
    ...
    private void firstFillCover(RecyclerView.Recycler recycler, RecyclerView.State state, int scrollDelta) {
        ...
        LogUtil.d(TAG, "firstFillCover finish:first: " + mFirstVisiblePosition + ",last:" + mLastVisiblePos);
    ...
    private float calculateToCenterFraction(View child, float pendingOffset) {
        ...
        LogUtil.d(TAG, "calculateToCenterFraction: distance:" + distance + ",childLength:" + childLength);
    ...
    private void fillWithVertical(RecyclerView.Recycler recycler, RecyclerView.State state, int dy) {
        LogUtil.d(TAG, "fillWithVertical: dy:" + dy);
            ...
            if (dy >= 0) {
                ...
                for (int i = 0; i < getChildCount(); i++) {
                    ...
                    if (getDecoratedBottom(child) - dy < topEdge) {
                        LogUtil.v(TAG, "fillWithVertical: removeAndRecycleView:" + getPosition(child) + ",bottom:" + getDecoratedBottom(child));

                        removeAndRecycleView(child, recycler);
                        mFirstVisiblePosition++;
                        fixIndex--;
                    } else {
                        LogUtil.d(TAG, "fillWithVertical: break:" + getPosition(child) + ",bottom:" + getDecoratedBottom(child));
                ...
                for (int i = getChildCount() - 1; i >= 0; i--) {
                    ...
                    if (getDecoratedTop(child) - dy > bottomEdge) {
                        LogUtil.v(TAG, "fillWithVertical: removeAndRecycleView:" + getPosition(child));
        ...
        if (dy >= 0) {
            ...
            for (int i = startPosition; i < getItemCount() && startOffset < bottomEdge + dy; i++) {
                ...
                startOffset = scrapRect.bottom;
                mLastVisiblePos = i;
                LogUtil.d(TAG, "fillWithVertical: add view:" + i + ",startOffset:" + startOffset + ",mLastVisiblePos:" + mLastVisiblePos + ",bottomEdge" + bottomEdge);
    ...
    private void fillWithHorizontal(RecyclerView.Recycler recycler, RecyclerView.State state, int dx) {
        int leftEdge = getOrientationHelper().getStartAfterPadding();
        int rightEdge = getOrientationHelper().getEndAfterPadding();
        LogUtil.v(TAG, "fillWithHorizontal() called with: dx = [" + dx + "],leftEdge:" + leftEdge + ",rightEdge:" + rightEdge);
            ...
            if (dx >= 0) {
                ...
                for (int i = 0; i < getChildCount(); i++) {
                    ...
                    if (getDecoratedRight(child) - dx < leftEdge) {
                        ...
                        mFirstVisiblePosition++;
                        fixIndex--;
                        LogUtil.v(TAG, "fillWithHorizontal:removeAndRecycleView:" + getPosition(child) + " mFirstVisiblePosition change to:" + mFirstVisiblePosition);
                ...
                for (int i = getChildCount() - 1; i >= 0; i--) {
                    ...
                    if (getDecoratedLeft(child) - dx > rightEdge) {
                        removeAndRecycleView(child, recycler);
                        mLastVisiblePos--;
                        LogUtil.v(TAG, "fillWithHorizontal:removeAndRecycleView:" + getPosition(child) + "mLastVisiblePos change to:" + mLastVisiblePos);
        ...
        if (dx >= 0) {
            if (getChildCount() != 0) {
                ...
                LogUtil.d(TAG, "fillWithHorizontal:to right startPosition:" + startPosition + ",startOffset:" + startOffset + ",rightEdge:" + rightEdge);
            }
            for (int i = startPosition; i < getItemCount() && startOffset < rightEdge + dx; i++) {
                ...
                startOffset = scrapRect.right;
                mLastVisiblePos = i;
                LogUtil.d(TAG, "fillWithHorizontal,layout:mLastVisiblePos: " + mLastVisiblePos);
            }
        } else {
            //dx<0
            if (getChildCount() > 0) {
                ...
                LogUtil.d(TAG, "fillWithHorizontal:to left startPosition:" + startPosition + ",startOffset:" + startOffset + ",leftEdge:" + leftEdge + ",child count:" + getChildCount());
            }
...
//        return 48;
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        LogUtil.d(TAG, "scrollHorizontallyBy: dx:" + dx + ",fixed:" + delta);
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        LogUtil.d(TAG, "scrollVerticallyBy: dy:" + dy + ",fixed:" + delta);
        getState().mScrollDelta = -delta;
        fillCover(recycler, state, -delta);
    ...
    private class InnerScrollListener extends RecyclerView.OnScrollListener {
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            if (snap != null) {
                ...
                if (selectedPosition != mCurSelectedPosition) {
                    ...
                    if (!mCallbackInFling && mState != SCROLL_STATE_IDLE) {
                        LogUtil.v(TAG, "ignore selection change callback when fling ");
            ...
            LogUtil.v(TAG, "onScrolled: dx:" + dx + ",dy:" + dy);
        }
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            mState = newState;
            LogUtil.v(TAG, "onScrollStateChanged: " + newState);
            if (mState == SCROLL_STATE_IDLE) {
                ...
                if (snap != null) {
                    ...
                    LogUtil.e(TAG, "onScrollStateChanged: snap null");
                }
            }
        }
    }
